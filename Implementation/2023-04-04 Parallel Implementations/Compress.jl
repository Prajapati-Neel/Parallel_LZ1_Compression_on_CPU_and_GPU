...
function Compress_eachindex(option_input)#(input,option)
...
function Compress_findall(option_input)#(input,option)
    input=read("Input_for_Compression.txt",String)
    option=UInt8(option_input)     #1 is for 12-bit, Max 4095  
    # option=UInt8(2)     #2 is for 16-bit, Max 65535
    search_buffer_length=4075
...
@btime Compress_findall(1)
# @btime Compress_eachindex(1)
# @btime Compress_eachindex(2)
