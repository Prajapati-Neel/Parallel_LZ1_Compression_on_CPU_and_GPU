...
function Uncompress_Parallel_Divided_Input()
    ...
    read_stream=open("Compressed_Parallel_Divided_Input.txt", "r")
    ...
    @threads for i in 1:no_of_chunks
        Output_strings[i]=Uncompress(option, positions_in_file[i], Lengths_of_Compressed_Chunks[i])
    end
    for i in 1:no_of_chunks
...
function Uncompress(option_input, position_in_file, Length_of_Compressed_Chunk)
    local_read_stream=open("Compressed_Parallel_Divided_Input.txt", "r")
...
@btime Uncompress_Parallel_Divided_Input()
